...
import PreviewDocumentWidget from "@/views/app/Ticket/widgets/PreviewDocumentWidget.vue";
...
export default {
  ...
  methods: {
    ...
    previewDocumentHandleClick(documentPreview) {
      this.previewDocument = documentPreview

      const routerData = this.$router.resolve({path: `/app/preview/document/${documentPreview.document_id}`,params:{document:documentPreview}})
      setTimeout(() => {
...
<!--      </div>-->
      <section>
        <DataTable v-if="ticket_id" :columns="columDataDocument" :url="`/tickets/documents?ticket_id=${ticket_id}`"
                   ...
                   :custom-actions="[{icon:'FeatherIcon',onClick:stampDocument, label:'Stamp'},{icon:'EyeIcon',onClick:previewDocumentHandleClick, label:'Preview'}]"
