<template>
  <div class="p-1 ticket cursor-pointer position-relative " :class="'ticket-'+deadlineColor" @click="$emit('moredetails')"
    ...
    <FloatingActionButton :key="itemOver.ticket_id" v-if="itemOver" class="position-absolute" :style="{top:topFloatElement,left:leftFloatElement}">
      <b-row slot="action">
        <b-col>
          <b-button @click.stop="$emit('assign')" v-b-tooltip.hover :title="$t('button~assignto')" size="sm" class="mr-2">
            <feather-icon icon="UserCheckIcon" size="15" class="text-white"/>
          </b-button>
          <b-button @click.stop="(e)=>toggleTicket(e,ticket)" size="sm" v-b-tooltip.hover
