...
<!--                    {{ $t('button~newsubtask') }}-->
<!--                  </b-button>-->
<!--                  &lt;!&ndash;                  <field :field="{key:'subticket', noLabel :true, required: false}" :entity="entity"/>&ndash;&gt;-->
<!--                </div>-->
<!--                <div class="mt-1">-->
...
export default {
  ...
  data() {
    return {
      deadline_red: moment(this.ticket.ticket_deadline_red),
      deadline_yellow: moment(this.ticket.ticket_deadline_yellow),
      column_deadline_red: moment(this.ticket.ticket_deadline_offset_red),
      column_deadline_yellow: moment(this.ticket.ticket_deadline_offset_yellow),
      itemOver: null,
      topFloatElement: 0,
      leftFloatElement: 0,
      deadlineColor: '',
  ...
  computed: {
    ...
    firstColumn() {
      return { column_id: this.ticket.column_id, column_name: this.ticket.column_name }
    },
    assignedUser() {
      // const column = this.ticket.columns[0]
      if (this.ticket.user_id_assigned) {
        if (this.ticket.user_firstname_assigned) {
          return `${this.ticket.user_firstname_assigned} ${this.ticket.user_lastname_assigned}`
        }
        return this.ticket.user_email_assigned
      }
      if (this.ticket.team_name) {
        return this.ticket.team_name
      }
      return 'None'
    },
    deadlineForHuman() {
