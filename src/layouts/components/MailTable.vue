    ...
    filteredEmail() {
      let prefiltered = this.items.filter(email => {
        ...
        if (this.filterValue === 1) {
          ...
          // if (email.documentModule.length > 0 && email.documentModule.every(d => d.ticket_created || d.classification_dismissed) && (!email.email_dismissed && !email.ticket_id_created)) return false
          return true
        }
        if (email.email_dismissed) return true
        if (email.ticket_id_created) return true
        // if (email.documentModule.every(d => d.ticket_created || d.classification_dismissed)) return true
        return false
      })
